...
export function createCache(adapter) {
  ...
  const cacheInstance = {
    ...
    async addExtra(key, extra) {
      ...
      const addedExtra = hasItem ? await adapter.addExtra(preData.key, preData.extra) : undefined;
    ...
    async setExtra(key, extra) {
      ...
      const hasItem = await this.hasItem(preData.key);
      const setExtra = hasItem ? await adapter.setExtra(preData.key, preData.extra) : undefined;
